...
void tpool_init(tpool_t* tpool_p, int num_thread ,int max_queue_size)
{
	...
	pool->num_thread = 0;
	pool->max_queue_size = max_queue_size+1; //why plus 1?
...
//init
	...
	for (i = 0; i < num_thread; i++)
	{
		...
		if (pthread_create(&pool->tid[i],NULL,thread_creat,(void*)pool)!=0)
		{
...
void *thread_creat(void* arg) //use for creating threads.
{
	...
	for(;;) //loop forever
	{
		...
		int is_full = full(pool);  //why here?
...
int tpool_add_work(tpool_t tpool,void(*process)(void*),void* arg)//I don't understand what it means. (void(*process)(void*))
{
	tpool_work_t *temp;
